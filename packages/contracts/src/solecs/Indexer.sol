...
contract Indexer is IEntityIndexer, IERC165 {
  ...
  mapping(bytes => EnumerableSet.UintSet) internal valueToEntities;
  ...
  function update(uint256 entity, bytes memory value) external {
    ...
    if (entityToValue[entity].length == 0) {
      ...
      for (uint256 i = 0; i < components.length; i++) {
        ...
        values[i] = components[i] == component ? value : components[i].getRaw(entity);
    ...
    // Remove old value
    valueToEntities[entityToValue[entity]].remove(entity);
    ...
    valueToEntities[indexerValue].add(entity);
  }
  ...
  function supportsInterface(bytes4 interfaceId) external view returns (bool) {
    return
      interfaceId == type(IERC165).interfaceId || interfaceId == type(IEntityIndexer).interfaceId;
  }
  ...
  function getEntitiesWithValue(bytes memory value) external view returns (uint256[] memory) {
    ...
    return valueToEntities[value].values();
  }
