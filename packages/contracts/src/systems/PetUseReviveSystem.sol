...
uint256 constant ID = uint256(keccak256("system.pet.use.revive"));

// eat one revive
contract PetUseReviveSystem is System {
  ...
  function execute(bytes memory arguments) public returns (bytes memory) {
    ...
    require(LibPet.isDead(components, petID), "pet not dead");
    ...
    LibPet.setLastTs(components, petID, block.timestamp); // explicitly, as we don't sync health on this EP

    // standard logging and tracking
    LibPet.logRevive(components, petID);
